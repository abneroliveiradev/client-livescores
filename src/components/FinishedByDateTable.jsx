...
function FinishedByDateTable() {
  ...
  useEffect(() => {
    ...
    socket.on("finishedGames", (message) => {
      setData([]);
      updateTable(message);
    });
  ...
  return (
    <div>
      <h3>Jogos Finalizados por Data</h3>
      <table
        style={{
      ...
      <button onClick={loadFinishedEvents}>Carregar Jogos Finalizados</button>
    </div>
  );
}

export default FinishedByDateTable;
